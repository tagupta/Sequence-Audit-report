...
contract Recovery is ISapientCompact {
  ...
  function _recoverBranch(
    ...
    while (rindex < _signature.length) {
      ...
      if (flag == FLAG_RECOVERY_LEAF) {
        ...
        //@note since this check is not dependent upon valid signer, can this be bypassed for malicious payload and verified can be set to true
      ...
      if (flag == FLAG_BRANCH) {
        ...
        //@report-written the recursion depth can grow excessively
  ...
  //@report-written with poc test => address 0 is passed for signer
  function queuePayload(
    ...
    //@audit-low unbounded growth of this array leading to storage bloat, as anyone can add any payload with no access control
  ...
  function isValidSignature(
    ...
    if (_signature.length == 64) {
      ...
      console2.log("address: ", addr, _signer);

      //@report-written can this be exploited for incorrect v,r,s and if signer is passed as address(0)
      if (addr == _signer) {
        return true;
      }
    }
